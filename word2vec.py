...
class Vocab:
    def __init__(self, fi, min_count, batch):
        vocab_items = []
        vocab_hash = {}
        word_count = 0
        fi = open(fi, 'r')
...
def __init_process(*args):
    global vocab, syn0, syn1, table, cbow, neg, dim, batch, starting_alpha
    global win, num_processes, global_word_count, fi
    
    vocab, syn0_tmp, syn1_tmp, table, cbow, neg, dim, batch, starting_alpha, win, num_processes, global_word_count = args[:-1]
    fi = open(args[-1], 'r')
...
def train(fi, fo, cbow, neg, dim, batch, alpha, win, min_count, num_processes, binary):
    # Read train file to init vocab
    vocab = Vocab(fi, min_count, batch)
    ...
    pool = Pool(processes=num_processes, initializer=__init_process,
                initargs=(vocab, syn0, syn1, table, cbow, neg, dim, batch, alpha,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-batch', help='Batch size', dest='batch', default=1, type=int)
    ...
    train(args.fi, args.fo, bool(args.cbow), args.neg, args.dim, args.batch, args.alpha, args.win,
